...
class PassCheckListener implements StepExecutionListener {
    @Override
    ...
    public ExitStatus afterStep(StepExecution stepExecution) {
        ...
        ExitStatus CUSTOM_EXIT_STATUS_CODE = getExitStatus(exitCode);
        if (CUSTOM_EXIT_STATUS_CODE != null) return CUSTOM_EXIT_STATUS_CODE;
        return null;
    }

    private static ExitStatus getExitStatus(String exitCode) {
        if (exitCode.equals(ExitStatus.FAILED.getExitCode())) {
            return new ExitStatus(DEFAILT_ENUM.CUSTOM_EXIT_STATUS_CODE.getValue());
        }
        return null;
    }
}
